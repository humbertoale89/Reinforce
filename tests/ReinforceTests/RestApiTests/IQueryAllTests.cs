...
namespace ReinforceTests.RestApiTests
{
    public class IQueryAllTests
    {
        ...
        public async Task IQueryAll(QueryResponse<string> expected)
        {
            ...
            var result = await api.GetAsync<string>(query, CancellationToken.None, "v56.0");
            result.Should().BeEquivalentTo(expected);
            handler.ConfirmPath("/services/data/v56.0/queryAll?q=Select+Id%2C+Name+From+Account+Where+Active__c+%3D+%27Yes%27");
        }
    }
}