...
namespace ReinforceTests.RestApiTests
{
    public class IRecordCountTests
    {
        ...
        public async Task IRecordCount_SObjects(RecordCountResponse expected)
        {
            ...
            var result = await api.GetAsync(sobjects, CancellationToken.None, "v56.0");
            result.Should().BeEquivalentTo(expected);
            handler.ConfirmPath($"/services/data/v56.0/limits/recordCount?sObjects={Uri.EscapeDataString(sobjects)}");
        }
    }
}